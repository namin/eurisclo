...
(defun try-apply-add (args &optional (alg-to-use *alg-to-use*) (cur-unit *cur-unit*))
  ...
  (let ((maybe-failed nil))
    (setf maybe-failed (handler-case
                           (bt:with-timeout ((max-real-time))
                             (apply alg-to-use args))
                         (bt:timeout ()
                           (cprin1 39 "try-apply-add failed: timeout in " cur-unit "~%")
                           '(failed))
                         (warning ()
                           (cprin1 39 "try-addply-add failed: warning in " cur-unit "~%")
                           '(failed))
                         (serious-condition (condition)
                           (cprin1 39 "try-apply-add failed: serious condition in " cur-unit " " condition "~%")
                           '(failed))
                         (:no-error (c) (list c))))
